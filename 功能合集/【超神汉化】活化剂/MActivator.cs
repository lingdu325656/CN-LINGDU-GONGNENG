...
{{
    ...
    {{
        ...
        // Jungle Minions
        MMinion blue = new MMinion("SRU_Blue", 蓝", 6, 143);
        MMinion red = new MMinion("SRU_Red", 红", 6, 143);
        MMinion dragon = new MMinion("SRU_Dragon", 小龙", 6, 143);
        MMinion baron = new MMinion("SRU_Baron", 大龙", -18, 192);
        MMinion wolf = new MMinion("SRU_Murk wolf", 三狼", 41, 74);
        MMinion razor = new MMinion("SRU_Razorbeak", F4", 39, 74); // Ghosts
        MMinion krug = new MMinion("SRU_Krug", 石像", 38, 80);
        MMinion crab = new MMinion("Sru_Crab", 螃蟹", 43, 62);
        MMinion gromp = new MMinion("SRU_Gromp", 幽灵", 32, 87); // Ghost
                        ...
                        {{
                            ...
                            {{
                                ...
                                {{
                                    if (item == ignite)
                                    {{
                                        // SimpleTS.TargetingMode.LowHP FIX/Check
                                        Obj_AI_Hero target = SimpleTS.GetTarget(item.range, SimpleTS.DamageType.Physical); // Check about DamageType
                                        if (target != null)
                                        {{
        ...
        {{
            ...
            SimpleTS.TargetingMode targetModeSelected = new SimpleTS.TargetingMode();

            foreach (SimpleTS.TargetingMode targetMode in Enum.GetValues(typeof(SimpleTS.TargetingMode)))
            {{
        ...
        {{
           ...
           Config.SubMenu("smiteCfg").AddItem(new MenuItem("dSmite", 显示")).SetValue(true);
            ...
            // Target selector
            Config.AddSubMenu(new Menu("目标选择", SimpleTS"));
            Config.SubMenu("SimpleTS"); // FIX/Test
            //Config.SubMenu("SimpleTS").AddItem(new MenuItem("targetMode", ")).SetValue(new StringList(new[] {{ LowHP", MostAD", MostAP", Closest", NearMouse", AutoPriority", LessAttack", LessCast" }, 0));
